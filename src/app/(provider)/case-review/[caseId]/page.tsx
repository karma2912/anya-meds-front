...
export default function CaseReviewPage() {
  ...
  const handleFinalizeReport = async () => {
    ...
    try {
        const response = await fetch(`/api/provider/cases/${analysis._id}/finalize`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ notes: providerNotes }),
        });
        ...
        // Refresh the page's data to show the new "completed" status
 ...
 const handleDownloadReport = async () => {
  ...
  try {
    ...
    if (logoBase64 && logoBase64 !== "linktobase64") {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="shadow-sm">
             ...
             <Card className="shadow-sm">
                <CardHeader><CardTitle>Provider Review & Notes</CardTitle></CardHeader>
                <CardContent>
                    <div>
                        <Label className="text-sm font-medium">Final Diagnosis & Recommendations</Label>
                        <Textarea 
                            id="providerNotes" 
                            placeholder="Enter your findings, confirm or override the AI diagnosis, and add clinical recommendations..." 
                            className="mt-2" 
                            rows={6}
                            value={providerNotes}
                            onChange={(e) => setProviderNotes(e.target.value)}
                            disabled={analysis.status === 'completed'} // Disable if already finalized
                        />
                    </div>
                </CardContent>
            </Card>
