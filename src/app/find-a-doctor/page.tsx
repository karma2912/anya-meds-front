...
import { Search, MapPin, Phone, Star, Clock, Crosshair } from 'lucide-react';
...
const FindDoctorPageContent = () => {
  ...
  const DoctorMap = useMemo(() => dynamic(() => import('@/components/DoctorMap'), { 
    loading: () => (
  ...
  useEffect(() => {
    const specialty = searchParams.get('specialty') || searchParams.get('specialty'); // Handle typo
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-blue-600 mb-2">Find a Specialist</h1>
          <p className="text-gray-600 text-md sm:text-lg">Search for doctors and clinics in your area</p>
        </div>
              ...
              <CardContent className="space-y-3 p-0">
                {filteredDoctors.length > 0 ? (
                  filteredDoctors.map(doctor => (
                    <div 
                      key={doctor.id} 
                      className={`p-4 border-b border-gray-100 hover:bg-blue-50 transition-colors cursor-pointer ${selectedDoctor?.id === doctor.id ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''}`}
                  ...
                  <div className="p-8 text-center">
                    ...
                    <p className="text-sm text-gray-400 mt-1">Try adjusting your search or specialty filter.</p>
                  </div>
                )}
              </CardContent>
            </Card>
